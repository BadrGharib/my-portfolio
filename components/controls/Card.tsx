...
function Card ({ imgs, title, description, link, logo, smallTitle }: CardInfo): ReactElement {
  const [selectedImg, setSelectedImg] = useState<string>('')

  useEffect(() => {
    (imgs != null) && imgs.length > 0 && setSelectedImg(imgs[0])
  }, [imgs])
  ...
  return (
    <div className="flex  flex-col w-full rounded-xl h-[100%] shadow-lg p-2 relative ">
      ...
      {smallTitle && (
        <p className="text-blue-800 italic text-sm">{smallTitle}</p>
      )}
      {link && (
        <Link
          className="text-blue-600 font-sans break-all"
          href={link}
          target="_blank"
          passHref
        >
      ...
      <p className="text- m-1 text-[14px] text-gray-600">{description}</p>
    </div>
  )
}
